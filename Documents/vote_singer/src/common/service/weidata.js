...
export default class extends think.service.base {
  ...
  async getJsSdk (self) {
    ...
    try {
      ...
      RES_INF.signature = hash(`jsapi_ticket=${RES_INF.data}&noncestr=${RES_INF.str}&timestamp=${RES_INF.timeStamp}&url=${'http://' + self.http.headers.host + self.http.req.url}`, 'sha1');
      return RES_INF;
    } catch (e) { 
      return false;
    }
  ...
  async getOpenid (self) {
    // let redirect_uri = self.http.headers.host + self.http.req.url;
